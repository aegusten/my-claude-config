...
```bash
find . -maxdepth 2 -name "*.md" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.claude/data/*" -not -path "./.context/*" | sort
```
...
```bash
gh pr view --json body -q .body > /tmp/iris-pr-body-$$.md
```
...
```bash
gh pr edit --body-file /tmp/iris-pr-body-$$.md
```

5. Clean up the tempfile:

```bash
rm -f /tmp/iris-pr-body-$$.md
```
