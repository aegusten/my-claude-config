...
DESIGN=$(ls -t ~/.claude/data/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.claude/data/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
mkdir -p ~/.claude/data/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.claude/data/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.claude/data/projects/$SLUG/ceo-plans/{old-plan}.md ~/.claude/data/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.claude/data/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
* **Effort estimate:** S/M/L/XL (human team) → with AI-assisted coding: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
echo '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE"}' >> ~/.claude/data/projects/$SLUG/$BRANCH-reviews.jsonl
```
...
```bash
eval $(~/.claude/bin/iris-slug 2>/dev/null)
cat ~/.claude/data/projects/$SLUG/$BRANCH-reviews.jsonl 2>/dev/null || echo "NO_REVIEWS"
echo "---CONFIG---"
~/.claude/bin/iris-config get skip_eng_review 2>/dev/null || echo "false"
```
...
- **B)** Keep in `~/.claude/data/projects/` only (local, personal reference)
- **C)** Skip
