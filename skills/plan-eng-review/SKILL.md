...
Write to `~/.claude/data/projects/{slug}/{user}-{branch}-test-plan-{datetime}.md`:
...
eval $(~/.claude/bin/iris-slug 2>/dev/null)
mkdir -p ~/.claude/data/projects/$SLUG
echo '{"skill":"plan-eng-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE"}' >> ~/.claude/data/projects/$SLUG/$BRANCH-reviews.jsonl
```
