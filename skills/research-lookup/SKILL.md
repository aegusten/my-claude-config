...
description: Look up current research information and web content using Perplexity Sonar models through OpenRouter. Automatically selects the best model based on query complexity. Search academic papers, recent studies, technical documentation, general research information with citations, and current web information beyond knowledge cutoff.
...
For general web searches (non-academic), use the perplexity_search.py script:

```bash
python scripts/perplexity_search.py "your query here"
```

**Available models via --model flag:**
- `sonar-pro` (default): General-purpose, best balance of cost and quality
- `sonar-pro-search`: Advanced agentic search with multi-step reasoning
- `sonar`: Most cost-effective for simple queries
...
Use web search mode for: current events, real-time information, non-scholarly sources, technology comparisons, and general fact-finding beyond knowledge cutoff.
