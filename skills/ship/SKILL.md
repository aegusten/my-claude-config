...
3. **If the user chooses A or C,** persist the decision so future `/ship` runs on this branch skip the gate:
   ```bash
   eval $(~/.claude/bin/iris-slug 2>/dev/null)
   echo '{"skill":"ship-review-override","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","decision":"USER_CHOICE"}' >> ~/.claude/data/projects/$SLUG/$BRANCH-reviews.jsonl
   ```
...
Check if the diff touches frontend files using `iris-diff-scope`:

```bash
eval $(~/.claude/bin/iris-diff-scope <base> 2>/dev/null)
```
...
echo '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M}' >> ~/.claude/data/projects/$SLUG/$BRANCH-reviews.jsonl
```
